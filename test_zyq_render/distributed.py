...
parser.add_argument('--mode', type=str, default='render', 
                choices=['ortho_gt', 'ortho_pos', 'ortho_random','ortho_depth','save_texture'],
...
def is_directory_empty(directory_path):
    ...
    :return: 如果文件夹为空，返回 True；否则返回 False
    """
    try:
        # 列出文件夹中的所有文件和子文件夹
...
def worker(
    ...
    while True:
        ...
        view_path = os.path.join(args.save_folder, item.split('/')[-1][:-4])
        ...
        if args.mode == 'ortho_gt':
            command = (
                f" CUDA_VISIBLE_DEVICES={gpu} "
                #f" blenderproc run --blender-install-path {args.blender_install_path} blenderProc_ortho.py"
                f" blenderproc run --blender-install-path {args.blender_install_path} ortho_gt_single.py"
        ...
        if args.mode == 'ortho_pos':
            command = (
                f" CUDA_VISIBLE_DEVICES={gpu} "
                #f" blenderproc run --blender-install-path {args.blender_install_path} blenderProc_ortho.py"
                f" blenderproc run --blender-install-path {args.blender_install_path} ortho_pos_single.py"
                f" --object_path {item} --view {view_idx}"
                f" --output_folder {args.save_folder}"
                f" --ortho_scale {args.ortho_scale}"
                f" --resolution {args.resolution} "
            ...
            if args.random_pose:
                print("random pose to render")
                command += f" --random_pose"
        if args.mode == 'ortho_random':
            command = (
                f" CUDA_VISIBLE_DEVICES={gpu} "
                #f" blenderproc run --blender-install-path {args.blender_install_path} blenderProc_ortho.py"
                f" blenderproc run --blender-install-path {args.blender_install_path} ortho_random_single.py"
                f" --object_path {item} --view {view_idx}"
                f" --output_folder {args.save_folder}"
                f" --ortho_scale {args.ortho_scale}"
                f" --resolution {args.resolution} "
            ...
            if args.random_pose:
                print("random pose to render")
                command += f" --random_pose"
        if args.mode == 'ortho_depth':
            command = (
                f" CUDA_VISIBLE_DEVICES={gpu} "
                #f" blenderproc run --blender-install-path {args.blender_install_path} blenderProc_ortho.py"
                f" blenderproc run --blender-install-path {args.blender_install_path} ortho_depth_single.py"
                f" --object_path {item} --view {view_idx}"
                f" --output_folder {args.save_folder}"
                f" --ortho_scale {args.ortho_scale} "
            )
        ...
        if args.mode == 'save_texture':
            command = (
                f" CUDA_VISIBLE_DEVICES={gpu} "
                f" {args.blender_install_path} -b -P texture_extract.py"
                f" --model_path {item}"
                f" --output_folder {args.save_folder}"
            )
        print(command)
...
if __name__ == "__main__":
    ...
    for item in model_paths[args.start_i:args.end_i]:
        if os.path.exists(os.path.join(args.objaverse_root, os.path.basename(item))):
            ...
            obj_path = os.path.join(args.objaverse_root, item+".glb")
        queue.put(obj_path)
