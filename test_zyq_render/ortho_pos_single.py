...
parser.add_argument('--blender-install-path', type=str, default='/disk2/ligs/blender/blender-3.5.1-linux-x64',help='blender install path')
...
def delete_useless_obj():
    ...
    for obj in scene_meshes():
        ...
        temp_l = max_corner.x - min_corner.x
        temp_w = max_corner.y - min_corner.y
        temp_h = max_corner.z - min_corner.z
        if temp_l > range_lw and temp_w > range_lw and temp_h < epsilon_h:
            # print(f"obj name{obj.name}, temp_l:{temp_l}, temp_w:{temp_w}, temp_h:{temp_h}")
...
#    print("dxyz: ",dxyz, "dist: ", dist)
    ...
    for obj in scene_root_objects():
        ...
        #obj.matrix_world.translation -= bbox_min
    bpy.ops.object.select_all(action="DESELECT")
...
VIEWS = [
        "_front", "_back", "_right", "_left", 
        "_front_right", "_front_left", "_back_right", "_back_left",
        "_front_top", "_back_top", "_right_top", "_left_top", 
        "_front_right_top", "_front_left_top", "_back_right_top", "_back_left_top",  
        ]
EXTRA_VIEWS = []
...
def save_images(object_file: str, viewidx: int) -> None:
    ...
    # 删除obj中独立的mesh比如底座，地面等
    delete_useless_obj()
    # 删除后再次normalize_scene
    scale, offset = normalize_scene()
    ...
    camera_locations = [
        ...
        np.array([0,-radius,radius]) / np.sqrt(3.),  # camera_front_top
        np.array([0.,radius,radius]) / np.sqrt(3.),  # camera back_top
        np.array([radius,0,radius]) / np.sqrt(3.),  # camera right_top
        np.array([-radius,0,radius]) / np.sqrt(3.), # camera left_top
    ...
    bproc.python.renderer.RendererUtility.set_render_devices(desired_gpu_device_type="CUDA")
    ...
    # 创建一个材质，输出世界坐标
    material = bpy.data.materials.new(name="WorldCoordsMaterial")
    ...
    # # 连接节点
    # links.new(geometry_node.outputs['Position'], emission_node.inputs['Color'])
    ...
    # import ipdb
    # ipdb.set_trace()
    # 设置第一个Vector Math节点为ADD，用来加1
    add_node.operation = 'ADD'
    add_node.inputs[1].default_value = -bbox_min #(1.0, 1.0, 1.0)  # 对x, y, z坐标都加1

    # 设置第二个Vector Math节点为DIVIDE，用来除以2
    divide_node.operation = 'DIVIDE'
    divide_node.inputs[1].default_value = bbox_max-bbox_min #(2.0, 2.0, 2.0)  # 对x, y, z坐标都除以2

    # 连接节点
    links.new(geometry_node.outputs['Position'], add_node.inputs[0])
    links.new(add_node.outputs['Vector'], divide_node.inputs[0])
    ...
    for j in range(len(VIEWS)):
        ...
        '{}/{}/normal_{}.webp'.format(args.output_folder, object_uid, index), "webp", quality=100)
        
        # position_map = data['colors'][index]
        # position_map = np.concatenate([position_map, valid_mask[:, :, None]], axis=-1)
        # Image.fromarray(position_map.astype(np.uint8)).save(
        # '{}/{}/pos_{}.webp'.format(args.output_folder, object_uid, index), "webp", quality=100)
